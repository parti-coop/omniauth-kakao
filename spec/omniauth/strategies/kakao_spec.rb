...
describe OmniAuth::Strategies::Kakao do
  ...
  subject(:middleware) do
    OmniAuth::Strategies::Kakao.new(app, { redirect_path: '/users/auth/kakao/callback' }.merge(@options || {})).tap do |strategy|
    ...
    it "should redirect to authorize page" do
      ...
      expect_url = <<~EXPECT.gsub(/(\n|\t|\s)/, '')
        https://kauth.kakao.com/oauth/authorize
          ?client_id=#{CLIENT_ID}
          &redirect_uri=http://#{SERVER_NAME}/users/auth/kakao/callback
          &response_type=code
        EXPECT
  ...
  describe "GET /users/auth/kakao/callback" do
    CODE = "dummy-code"
    STATE = "dummy-state"
    ...
    it "should request access token and user information" do
      request = make_request("/users/auth/kakao/callback?code=#{CODE}&state=#{STATE}", {
        'rack.session' => {
      ...
      expect(information.profile_image).to eq "http://xxx.kakao.com/.../bbb.jpg"
  ...
  context "test environment" do
    ...
    describe "GET /users/auth/kakao/callback" do
      it "should request registered mock" do
        request = make_request("/users/auth/kakao/callback")
        code, env = middleware.call(request)
        expect(code).to eq 200
